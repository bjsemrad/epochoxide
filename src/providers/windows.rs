...
struct SwayNode {
    ...
    window_properties: Option<SwayProps>,
...
impl Provider for WindowsProvider {
        ...
        for window in discover_windows() {
            ...
            if let Some((score, info)) = fuzzy::score(query, &haystack, exact, "text") {
                let mut item = Item::new(
                    self.name(),
                    format!("{}:{}", window.backend, window.id),
                    window.title,
                );
                item.subtext = format!("{} {}", window.app, window.workspace)
                    .trim()
                    .to_string();
    ...
    fn activate(
        ...
        let Some((backend, id)) = identifier.split_once(':') else {
            return Ok(());
        };
        match (action, backend) {
            ("focus", "hypr") => {
                hypr_window_dispatch("focuswindow", "hl.dsp.focus({ window = w })", id)
            }
            ("focus", "sway") => run_shell(&format!("swaymsg '[con_id={id}] focus'")),
            ("focus", "niri") => run_shell(&format!(
                "{} msg action focus-window --id {id}",
                niri_shell()
            )),
            ("focus", "wmctrl") => run_shell(&format!("wmctrl -ia {id}")),
            ("close", "hypr") => {
                hypr_window_dispatch("closewindow", "hl.dsp.window.close({ window = w })", id)
            }
            ("close", "sway") => run_shell(&format!("swaymsg '[con_id={id}] kill'")),
            ("close", "niri") => run_shell(&format!(
                "{} msg action close-window --id {id}",
                niri_shell()
            )),
...
fn collect_sway(node: &SwayNode, out: &mut Vec<Window>) {
    ...
    if let Some(nodes) = &node.floating_nodes {
        for n in nodes {
            collect_sway(n, out);
        }
    }
