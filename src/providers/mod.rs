...
impl Registry {
    pub fn new(config: Config) -> Result<Self> {
        ...
        if enabled(&config, "files") {
            providers.push(Box::new(files::LazyFilesProvider::new(config.clone())));
        }
        if enabled(&config, "runner") {
            providers.push(Box::new(runner::RunnerProvider::new(config.clone())));
        }
        if enabled(&config, "clipboard") {
            providers.push(Box::new(clipboard::ClipboardProvider::new(config.clone())?));
        }
        if enabled(&config, "windows") {
            providers.push(Box::new(windows::WindowsProvider::new(wm_class_icons)));
        }
        if enabled(&config, "calc") {
            providers.push(Box::new(calc::CalcProvider::new(config.clone())));
        }
        ...
        if enabled(&config, "menus") {
            for menu in menus::providers(&config)? {
                if !enabled(&config, menu.name()) {
                    continue;
                }
                if providers.iter().any(|p| p.name() == menu.name()) {
                    continue;
                }
                providers.push(menu);
            }
        ...
        self.capabilities
            ...
            .map(|cap| {
                ...
                cap.prefixes = config
                    .query_prefixes
                    .iter()
                    .filter(|(_, provider)| provider == &&cap.name)
    ...
    fn targets(&self, providers: &[String]) -> Vec<usize> {
        ...
        (0..self.providers.len())
            .filter(|&i| include_all || providers.iter().any(|p| p == &self.capabilities[i].name))
            .collect()
    }
    ...
    pub fn query(&self, providers: &[String], query: &str, limit: usize, exact: bool) -> Vec<Item> {
        ...
        for item in &mut out {
            ...
            if let Some(ref p) = icon_path {
                item.thumbnail = icons::thumbnail(p, &config);
            }
        }
        out
    }
    ...
    pub fn menu(&self, name: &str) -> Vec<Item> {
        let Some(idx) = self.capabilities.iter().position(|c| c.name == name) else {
            return Vec::new();
        };
