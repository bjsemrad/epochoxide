...
impl ClipboardStore {
    ...
    fn capture_text(&self) -> Result<bool> {
        ...
        if items.first().map(|i| i.content.as_str()) == Some(text.as_str()) {
            return Ok(true);
        }
    ...
    fn capture_image(&self, mime: &str) -> Result<()> {
        ...
        let mut items = self.items.lock().unwrap();
        if items.first().map(|i| i.id.as_str()) == Some(id.as_str()) {
            return Ok(());
        }
        ...
        let ocr = if self.config.clipboard_ocr {
            run_ocr(&path).unwrap_or_default()
        } else {
            String::new()
        };
        items.retain(|i| i.id != id);
        items.insert(
            ...
            Clip {
                id,
                kind: ClipKind::Image,
                content: if ocr.is_empty() {
                    "Image clipboard item".into()
                } else {
                    ocr.lines()
                        .next()
...
impl ClipboardProvider {
    pub fn new(config: Config) -> Result<Self> {
        let cache = dirs::cache_dir()
            ...
            .join("epochoxide/clipboard.json");
        let items = fs::read_to_string(&cache)
            .ok()
...
impl Provider for ClipboardProvider {
        ...
        for (index, clip) in items.iter().enumerate() {
            ...
            if let Some((score, info)) = fuzzy::score(query, &searchable, exact, "text") {
                let mut item = Item::new(
                    self.name(),
                    &clip.id,
                    clip.content.trim_start().lines().next().unwrap_or_default(),
                );
                ...
                item.icon = match clip.kind {
                    ClipKind::Text => "edit-paste",
                    ClipKind::Image => "image-x-generic",
                }
                .into();
                item.actions = vec![
                    "copy".into(),
                ...
                if clip.kind == ClipKind::Image && self.store.config.clipboard_ocr {
                    item.actions.push("ocr".into());
                }
                if clip.pinned {
                    item.state.push("pinned".into());
                }
                item.state.push(
                    match clip.kind {
                        ClipKind::Text => "text",
    ...
    fn activate(
        ...
        match action {
            "copy" => {
                let items = self.store.items.lock().unwrap();
                if let Some(c) = items.iter().find(|i| i.id == identifier) {
                    copy_clip(c)?;
                }
                Ok(())
            }
            "edit" => {
                let index = self
                    ...
                    .unwrap()
                    .iter()
                    .position(|i| i.id == identifier);
                if let Some(index) = index {
                    self.edit_clip(index)?;
                }
                Ok(())
            }
            "ocr" => {
                ...
                if let Some(c) = items.iter_mut().find(|i| i.id == identifier) {
                    if let Some(path) = &c.image_path {
                        c.ocr = run_ocr(Path::new(path)).unwrap_or_default();
                    }
                }
    ...
    fn capability(&self) -> ProviderCapability {
        ProviderCapability {
            ...
            actions: action_map(&[
                ...
                (
                    "remove_all",
                    ActionCapability::new("Remove All").destructive(),
                ),
            ]),
            supports_query: true,
    ...
    fn edit_clip(&mut self, index: usize) -> Result<()> {
        ...
        match kind {
            ClipKind::Text => {
                ...
                items[index].id =
                    format!("text-{:x}", stable_hash(items[index].content.as_bytes()));
            ...
            ClipKind::Image => {
                ...
                let editor = if self.store.config.clipboard_image_editor.is_empty() {
                    "xdg-open"
                } else {
                    &self.store.config.clipboard_image_editor
                };
