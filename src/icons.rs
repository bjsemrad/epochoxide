...
impl IconResolver {
    pub fn new(config: &Config) -> Self {
        Self { search_dirs: theme_search_dirs(config), cache: Mutex::new(HashMap::new()) }
    }

    pub fn resolve(&self, icon: &str) -> Option<String> {
        if icon.is_empty() { return None; }
        let icon_path = Path::new(icon);
        if icon_path.is_absolute() { return icon_path.exists().then(|| icon.to_string()); }

        if let Some(cached) = self.cache.lock().unwrap().get(icon) { return cached.clone(); }
        let names = icon_names(icon);
        let found = self.search_dirs.iter()
            .find_map(|dir| names.iter().map(|name| dir.join(name)).find(|p| p.exists()))
            .map(|p| p.display().to_string());
        self.cache.lock().unwrap().insert(icon.to_string(), found.clone());
        found
    }
}
...
fn theme_search_dirs(config: &Config) -> Vec<PathBuf> {
    ...
    if let Some(data) = dirs::data_dir() { theme_bases.push(data.join("icons")); }
    if !config.icon_cache_dir.is_empty() { theme_bases.push(PathBuf::from(&config.icon_cache_dir)); }
    theme_bases.push(PathBuf::from("/usr/share/icons"));

    let start = if config.icon_theme.is_empty() { "hicolor" } else { config.icon_theme.as_str() };
    let mut queue = vec![start.to_string()];
    let mut seen = HashSet::new();
    let mut dirs = Vec::new();
    while let Some(theme) = queue.pop() {
        if !seen.insert(theme.clone()) { continue; }
...
struct ThemeIndex { inherits: Vec<String>, directories: Vec<String> }
...
fn read_theme_index(theme_dir: &Path) -> Option<ThemeIndex> {
    ...
    for line in raw.lines().map(str::trim) {
        if line == "[Icon Theme]" { in_icon_theme = true; continue; }
        if line.starts_with('[') { in_icon_theme = false; continue; }
...
mod tests {
    ...
    fn resolver_caches_after_first_lookup() {
        ...
        fs::write(dir.path().join("MyTheme/index.theme"), "[Icon Theme]\nDirectories=48x48/apps\n").unwrap();
        fs::write(apps_dir.join("firefox.png"), b"").unwrap();

        let config = Config { icon_theme: "MyTheme".into(), icon_cache_dir: dir.path().display().to_string(), ..Config::default() };
        let resolver = IconResolver::new(&config);

        let found = resolver.resolve("firefox").expect("should find icon on first lookup");
        fs::remove_file(apps_dir.join("firefox.png")).unwrap();
        let cached = resolver.resolve("firefox").expect("cached result should survive file removal");
